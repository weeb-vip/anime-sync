...
func EventingAnimeNats() error {
	...
	processorInstance = processorInstance.
		...
		AddMiddleware(backoffretry.NewBackoffRetry[anime_processor.Payload](retryDriver, backoffretry.Config{
	...
	retryProcessorInstance := processor.NewProcessor[*epNats.Message, anime_processor.Payload](retryDriver, retrySubject, postgresProcessor.Process).
		AddMiddleware(NewNatsLoggerMiddleware[anime_processor.Payload]().Process).
		AddMiddleware(NewNatsTransformMiddleware[anime_processor.Payload]().Process).
		AddMiddleware(backoffretry.NewBackoffRetry[anime_processor.Payload](retryDriver, backoffretry.Config{
			MaxRetries: maxRetries,
